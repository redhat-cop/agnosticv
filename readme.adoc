...
===== Common files in case of includes =====
...
The `#include` directive supports a `recursive` parameter that controls whether includes within the included file are processed:

* **`recursive=true`** (default) - Process `#include` directives within the included file
** The file's associated `.meta` file is always included
** `#include` directives in both the main file and its `.meta` file are processed
** If the file is a catalog item, its common files are included (`common.yaml`, `account.yaml`)
* **`recursive=false`** - Include the file but do NOT process any `#include` directives within it
** The file's associated `.meta` file is still always included
** File's common files are not included (`common.yaml`, `account.yaml`)
** `#include` directives in both the main file and its `.meta` file are NOT processed
...
**Important:** Meta files (`.meta.yaml` or `.meta.yml`) are always automatically included for any file, regardless of the `recursive` parameter. The `recursive` parameter only controls whether `#include` directives within those meta files are processed.
...
#include recursive=true /base.yaml             # Explicit recursive (same as default)
----
...
* But the `#include /third.yaml` directive inside `second.yaml` is NOT processed
* So `third.yaml` is NOT included
* Final result: `foo: from-first` (current file overrides the included second.yaml)

**Note:** If any of these files had associated `.meta.yaml` files, they would all be included in the merge, but any `#include` directives within those meta files would not be processed due to `recursive=false`.
...
* This happens regardless of whether the file is a catalog item, common file, or included file
* Even with `#include recursive=false`, the meta file is still included
* The `recursive` parameter only controls whether `#include` directives **within** the meta file are processed
