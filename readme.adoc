...
* _Catalog item_, or _Leaf_: a file that is considered as a catalog item. It will appear when listing an agnosticv repository with `agnosticv --list`. To print the content of a catalog item, you run `agnosticv --merge PATH`. That will merge common files, meta file, included files, and finally the leaf in order to produce the catalog item.
...
Files that are included in the merge list using the `#include PATH` feature. See dedicated section below.

Usually they are placed in an `includes/` directory at the top of the agnosticv repository.
...
** That's also a reason why you should put all your includes at the top of the file.
...
The meta file can contain the value of the  `\\__meta__` dictionary. It is a convenient way to separate that special dictionary from the other variables.

For example, the following files are valid meta files:

* `common.meta.yml` meta file for `common.yml`
* `common.meta.yaml` meta file for `common.yaml`
* `account.meta.yml` meta file for `account.yml`
* `account.meta.yaml` meta file for `account.yaml`
* `dev.meta.yml` meta file for `dev.yml`
* `dev.meta.yaml` meta file for `dev.yaml`

WARNING: you can only put the content of the `\\__meta__` variable in a meta file.

NOTE: the extension has to match the extension of the included file.
...
* Any directory `includes` is reserved to includes. The files in those directory are not considered as catalog items.
* Any file containing:
+
----
...
----
+
is ignored. It is not considered as a catalog item.
...
It is possible to extend agnosticV and define what merge strategy to use on what variable or part of dictionary variable.

To do that, you can define the custom merge strategies in any schema in the `.schemas` directory. Just use the **`x-merge`** keyword at the beginning of the schema. `x-merge` is a list of strategy. Each strategy defines a `path` and a `strategy` name to apply on that path.
...
properties:
----
<1> The path of the variable or key of dictionnary to apply the custom strategy against.
